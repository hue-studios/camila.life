...
<div id="page-container-box">
  ...
  <nav id="nav-bar" @click.prevent="closeSideMenu()">
    ...
    <p>
      <nuxt-link to="/account/">ACCOUNT <span class="uk-badge">{{$store.state.list.length + $store.state.recipes.length}}</span></nuxt-link>
    </p>
    ...
    <vk-grid class="uk-flex uk-flex-center uk-flex-top uk-grid-small uk-text-center uk-margin-top account-btns">
      <div>
        ...
        <div class="uk-width-1-1 uk-margin-small-top">
          <nuxt-link class="uk-button uk-button-default uk-text-center white" to="/account/logout">LOGOUT</nuxt-link>
        </div>
      </div>
    </vk-grid>
  </nav>
  ...
  <nuxt-link to="/account/logout/" id="log-btn" class="condensed">LOGOUT</nuxt-link>
  <toolbar />
</div>
</template>
<script>
import toolbar from '~/components/loggedIn/toolbar'
import {
  mapState
} from 'vuex'
