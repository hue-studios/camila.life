...
<template>
        ...
        <div class="ingredient-swiper-container">
          <div v-swiper:ingredientSwiper="swiperOption" class="ingredient-swiper">
            <div class="swiper-wrapper">
              <div class="swiper-slide" v-for="(image, index) in images" v-bind:key="index"> <img :src="'https://huestudios.com' +image.url" :alt="ingredient.name" :name="ingredient.name" data-swiper-parallax="100" /> <i data-swiper-parallax="-100" class="green fa fa-arrows-alt full-screen" aria-hidden="true" @click.prevent="showImageModal(image.url, image.name)" v-if="isAuthenticated"></i></div>
            </div>
  ...
  <h5 class="small-11 cell script-font text-lowercase goes-with" v-if="ingredient.goes_with.length > 0"><span class="green">goes with</span> <span class="goes-with-item" v-for="(goes, index) in goesWith" v-bind:key="index"> {{goes}}, </span><span v-if="recipes.length > 0">and <span class="green">featured in</span> {{recipes.length}} recipe<span v-if="recipes.length > 1">s</span></span>.</h5>
...
export default {
  ...
  methods: {
    ...
    showImageModal (image, name) {
      this.modalImage = 'https://huestudios.com' + image
      this.modalName = name
      this.showModal = true
    },
    closeModal () {
