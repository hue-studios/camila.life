...
<div id="list-screen" class="close">
  ...
  <div id="list-box"> 
    <div id="mailing-list">
      ...
      <transition enter-active-class="animated fadeInLeft" leave-active-class="animated fadeOutRight" mode="out-in">
        <form id="mailing-list-form" v-if="!responseBox">
          ...
          <button v-on:click.prevent="submitList()">SUBMIT</button>
        </form>
        <div id="response-box" class="text-center expanded row align-middle align-center" v-else>
          <div class="columns small-12">
            <p class="">Alright <span class="pink">{{name}}</span>!! <br>
              <br>
              You are now subscribed to Camila's mailing list!</p>
    ...
    <a id="close-btn" @click.prevent="closeForm">
      <div id="close-box"><span></span><span></span></div>
    </a>
  </div>
</div>
</template>
