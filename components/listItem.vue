  ...
  methods: {
    removeFromList (product) {
      axios.post('https://huestudios.com/sites/camila.life/scripts/list.php?email=' + this.$store.state.user.email + '&product_id=' + product + '&action=delete').then(res => {
        ...
        this.showDeleteMsg({
          title: `<span class='condensed' style='font-size: 20px; line-height: 20px; font-weight: 100;'>removed <span class='condensed-bold'>` + this.ingredient.name + `</span> FROM SHOPPING LIST</span>`,
          image: 'https://huestudios.com' + this.ingredient.images.data[0].url
        })
...
.list-item {
    ...
    border-radius: 3px;
    box-shadow: 0 1px 15px rgba($black, 0.3);
    margin-bottom: 40px;
    align-items: center;
